...
_BASE = Path(tempfile.gettempdir()) / "any2api-grok"
DEFAULT_PROFILE_ROOT = _BASE / "same_session_profiles"
...
def encode_grpc_create_email(email: str, castle_request_token: str) -> bytes:
    payload = _encode_grpc_string(1, email) + _encode_grpc_string(
        3, castle_request_token
    )
...
def _extract_router_state_tree(html: str) -> str:
    """Extract and encode the current App Router tree from Next.js Flight data."""
    for raw_segment in _RSC_PUSH_RE.findall(html or ""):
        try:
            segment = json.loads(f'"{raw_segment}"')
        except json.JSONDecodeError:
            continue
        for record in segment.splitlines():
            _, separator, payload = record.partition(":")
            if not separator or not payload.startswith("{") or '"f"' not in payload:
                continue
            try:
                flight = json.loads(payload).get("f")
            except (AttributeError, json.JSONDecodeError):
                continue
            if (
                not isinstance(flight, list)
                or not flight
                or not isinstance(flight[0], list)
                or not flight[0]
            ):
                continue
            tree = flight[0][0]
            if (
                ...
                or len(tree) < 2
                or tree[0] != ""
                or not isinstance(tree[1], dict)
                or "children" not in tree[1]
            ):
                continue
...
def _refresh_action_from_page(page, cfg: dict[str, str], log=None) -> dict[str, str]:
    ...
    # state tree: current Next.js renders it in self.__next_f Flight data.
    state_tree = _extract_router_state_tree(html)
    if state_tree:
        cfg["state_tree"] = state_tree
    ...
    for src in scripts:
        try:
            ...
            if not cfg.get("site_key"):
                site_match = re.search(r"(0x4AAAAA[0-9A-Za-z_-]{8,})", txt)
                if site_match:
                    cfg["site_key"] = site_match.group(1)
            if "createUserAndSessionRequest" in txt and "emailValidationCode" in txt:
                candidates = sorted(set(re.findall(r'"([a-fA-F0-9]{42})"', txt)))
                if len(candidates) == 1:
                    action = candidates[0]
                elif candidates:
                    action = next(
                        (value for value in candidates if value.startswith("7f")),
                        None,
                    )
            tos_match = re.search(r'TOS_ACCEPTED_VERSION",0,(\d+)', txt)
            if tos_match:
                tos_accepted_version = tos_match.group(1)
        except Exception:
            continue
    ...
    if tos_accepted_version:
        cfg["tos_accepted_version"] = tos_accepted_version
    # 未命中时静默沿用缓存，不刷屏
    ...
    *,
    ...
    try:
        ...
        try:
            ...
            mint_args = mint_js_args(email)  # 默认 fiber-only，短 token 不外传
            t_mint = time.time()
            ...
            if do_ts_parallel:
                sk = (cfg.get("site_key") or "").strip()
                if not sk:
                    out["error"] = "live Turnstile sitekey missing"
                    return out
            ...
            r1 = _page_binary_post(
                ...
                encode_grpc_create_email(email, castle),
                grpc_headers,
            )
            ...
            _stage("form_submitted")
            _stage("upstream_accepted")
            _stage("otp_ui_visible")
            ...
            if not code:
                ...
                return out
            code = str(code).strip()
            out["steps"].append("code:received")
            _log("验证码已收到")
            _stage("otp_received")
            ...
            raw3 = page.evaluate(_MINT_JS, mint_js_args(email))
            ...
            if not cfg.get("action_id"):
                ...
                return out
            tos_accepted_version = str(cfg.get("tos_accepted_version") or "").strip()
            if not tos_accepted_version.isdigit():
                out["error"] = "live Grok TOS accepted version missing"
                return out
            body = {
            ...
            for s_try in range(1, max_signup_tries + 1):
                if s_try > 1:
                    ...
                    # 再 mint castle
                    try:
                        raw_r = page.evaluate(_MINT_JS, mint_js_args(email))
            ...
            if post_sso is not None:
                out["post_sso"] = post_sso(page, ctx, sso, sso_rw)
                out["steps"].append("post_sso_ok")
        finally:
