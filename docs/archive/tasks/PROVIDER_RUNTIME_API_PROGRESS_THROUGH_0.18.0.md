# Provider Runtime/API 历史任务与验收快照（0.18.0 及以前）

> 本文件保留旧 `tasks/current.md` 的任务与运行态记录，并调整标题及相对链接。段落中的“当前”只对应记录时点，
> 不能作为 0.21.0 的源码或生产状态。

## API Key 通道选择与全厂商 API Channel（0.18.0）
...
- 公共框架职责与 Provider 职责已整理到 [ADR-0008](../../adr/0008-provider-boundaries-and-arena-direct.md)：框架负责 Action/Channel 契约、账号租约、浏览器预算、邮箱策略、生命周期、错误模型和观测；Provider 负责厂商协议、页面脚本、认证、媒体上传、事件解码和 provider-local 重试。
