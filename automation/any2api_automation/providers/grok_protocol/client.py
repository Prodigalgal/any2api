...
def _next_script_urls(html: str) -> list[str]:
    parser = _ScriptSourceParser()
    parser.feed(html or "")
    result: list[str] = []
    for source in parser.sources:
        absolute = urljoin("https://accounts.x.ai/", source)
        parsed = urlparse(absolute)
        if parsed.scheme != "https" or parsed.hostname != "accounts.x.ai":
            continue
        if not parsed.path.startswith("/_next/static/chunks/"):
            continue
...
class XConsoleAuthClient:
    ...
    def _scrape_action_id(self, html: str) -> str:
        ...
        js_urls = _next_script_urls(html)
        if self.debug:
        ...
        if not ordered:
            raise RuntimeError("The live sign-up page contains no Next.js script chunks")
        ...
        def _fetch_and_search(url: str) -> Tuple[Optional[str], bool]:
            """Return (hash_or_None, is_signup_chunk)."""
            try:
                _s, _h, _sc, raw = self._request("GET", url, headers=self._base_headers())
                ...
                if is_signup and self.debug:
                    print(f"  [scrape] SIGN-UP ACTION CHUNK: {urlparse(url).path}")
