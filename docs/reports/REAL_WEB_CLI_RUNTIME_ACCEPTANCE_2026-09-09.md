...
| 代码修复提交 | `0f0f931`、`88b681d`、`071cf19`、`297f871`；最后一项为 `fix: select wrapped minmax request bridge` |
| CI | GitHub Actions `Build and deploy` run `34297137313`，质量检查、三张业务镜像构建和 GitOps 更新均成功 |
| 代码制品镜像 | `server-sha-297f871f6e4df1577b623fdffc9feecaaf13406b`、`automation-sha-297f871f6e4df1577b623fdffc9feecaaf13406b`、`web-sha-297f871f6e4df1577b623fdffc9feecaaf13406b` |
| Argo CD | Application `any2api` 已 `Synced / Healthy / Succeeded`；本次代码制品同步 revision 为 `a8e60b1c8828962104622cac5841d9fe45c71b71` |
...
| MinMax Web/CLI | K8S 真实探测已由 Camoufox 建立 session；`/archon/api/v1/agent` 返回 200，创建 session 返回 200，`/archon/api/v1/session/{session_id}/message` 返回 200 `text/event-stream`；`MiniMax-M3` 和 `MiniMax-M2.7` 均收到上游 `42212 Token Plan usage limit`，服务归类 `quota_exhausted` | image | 31 个账号均未启用，其中 30 个 `PENDING`、1 个 `DISABLED`；多账号探测均无可用 Token Plan | 浏览器 Runtime 已打通；厂商仍未 Ready，需准备启用且有额度的账号 |
...
`MinmaxProvider.accountProbeTimeout()` 现在显式使用 2 分钟，并由 `MinmaxProtocolTest` 固定该契约。随后真实探测发现当前 bundle 的官方请求模块是直接函数或封装导出，旧发现逻辑无法选出桥；`88b681d`、`071cf19` 和 `297f871` 分别补充了脱敏网络诊断、模块导出兼容和封装函数回退。最终 K8S 日志证明 Camoufox 页面已经完成控制请求、会话创建和 SSE 聊天请求，所有请求都返回上游 200；失败只发生在上游内容中的 Token Plan 配额限制。

## 当前全局阻断项

1. MiMo：现有 Web 会话凭据被上游 401 拒绝，必须提供新的有效登录会话或完成受控重新认证。
2. MinMax：浏览器请求桥已打通，但当前没有启用账号，两个不同模型/账号样本都被上游 Token Plan 配额拒绝；需要准备有额度且允许启用的账号。
...
- MinMax 账号探测超时从接口默认值提升到 2 分钟，并兼容当前官方 bundle 的直接或封装请求桥导出；Backend 全量 `gradlew test` 通过，Automation 全量 `pytest`、`ruff format --check` 和 `ruff check` 通过，Web CI 构建通过。

## 发布判断

当前发布可以证明“统一浏览器 Runtime 代码已在 K8S 运行，并且 MinMax 已完成从页面桥到上游 SSE 的真实链路验证”，不能证明“全厂商、全流程、全业务、全多模态 Ready”。在 MiMo 凭据、MinMax 有额度账号和 Grok 启用条件未解决前，不应把系统状态改成全绿，也不应继续扩大账号激活范围。

## 回滚点

本次 MinMax 变更包含账号探测等待窗口、官方桥诊断和导出兼容；如需回滚代码，可回到上一已构建并通过 CI 的 `a9f0296` 镜像/GitOps 制品，但会重新引入已定位的 MinMax 桥发现和 30 秒探测问题。回滚仍应通过 GitOps 提交不可变镜像 tag，不直接编辑生产 Deployment。
