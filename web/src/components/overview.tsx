...
export function Overview() {
  ...
  const rows = groupProviders(models.data?.data ?? []);
  ...
  return (
    <Box sx={{ px: { xs: 1.5, sm: 2.5, lg: 3.5 }, py: { xs: 2, sm: 3 }, maxWidth: 1440, mx: "auto" }}>
      ...
      >
        ...
        <Button variant="outlined" startIcon={<RefreshOutlined />} onClick={() => void Promise.all([models.refetch(), health.refetch()])}>刷新状态</Button>
      </Stack>

      {models.error && <Alert severity="warning" sx={{ mb: 2 }}>后端尚未连接，启动 Java 服务后将显示真实厂商目录。</Alert>}
      ...
      <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", xl: "minmax(0, 2fr) minmax(300px, 1fr)" }, gap: 2.5 }}>
        <Paper variant="outlined" sx={{ overflow: "hidden" }}>
          <Box sx={{ px: 2, py: 1.75 }}>
            ...
            <Typography color="text.secondary" sx={{ fontSize: 12, mt: 0.25 }}>统一模型目录只展示当前 Java Provider Registry 的声明</Typography>
          </Box>
          <Divider />
          <TableContainer>
            <Table size="small">
              <TableHead><TableRow><TableCell>厂商</TableCell><TableCell>状态</TableCell><TableCell>默认模型</TableCell><TableCell>工具</TableCell><TableCell>多模态</TableCell></TableRow></TableHead>
              <TableBody>
                ...
                {!models.isLoading && rows.length === 0 && <TableRow><TableCell colSpan={5} align="center" sx={{ py: 5, color: "text.secondary" }}>等待后端模型目录</TableCell></TableRow>}
              </TableBody>
            </Table>
          </TableContainer>
        </Paper>
