...
_MINT_JS = r"""
async (args) => {
  // args: {pk, mode, minLen, request}  兼容旧调用只传 pk 字符串
  ...
  const request = (typeof args === 'object' && args && args.request)
    ? args.request : {method: 'email_password', flow: 'signup'};
  ...
  async function tryWindowCastle() {
    try {
      if (window.Castle && typeof window.Castle.createRequestToken === 'function') {
        const t = await window.Castle.createRequestToken(request);
  ...
  async function tryOfficialCdn() {
    try {
      ...
      if (typeof window.Castle.createRequestToken === 'function') {
        t = await window.Castle.createRequestToken(request);
        ...
        t = await window.Castle.default.createRequestToken(request);
      }
...
def mint_js_args(email: str = "") -> dict[str, Any]:
    """page.evaluate(_MINT_JS, mint_js_args()) 统一入参。"""
    request = {"method": "email_password", "flow": "signup"}
    if email.strip():
        request["email"] = email.strip()
    return {
        "pk": CASTLE_PK,
